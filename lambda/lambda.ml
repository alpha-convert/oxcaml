...
let lfunction' ~kind ~params ~return ~body ~attr ~loc ~mode ~ret_mode =
  ...
  | (Alloc_heap | Alloc_external), Tupled -> () (* CR jcutler: guess that this works the same as heap. *)
  | Alloc_local, Tupled ->
  ...
  | mode, Curried {nlocal} ->
     ...
     (* CR jcutler: this feels like a place where the compiler assumes that all
        allocations are either heap or local. I'd assume for the purposes of
        this, external behaves like heap... *)
...
let may_allocate_in_region lam =
  ...
  and loop = function
    ...
    | Lfunction {mode=Alloc_heap | Alloc_external} -> () (* CR jcutler: this probably shouldn't be reachable. *)
