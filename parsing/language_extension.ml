...
let get_level_ops : type a. a t -> (module Extension_level with type t = a) =
  ...
  | External_alloc -> (module Maturity)
...
let is_erasable : type a. a t -> bool = function
  ...
  | Separability | Let_mutable | External_alloc ->
    false
...
module Exist_pair = struct
  ...
  let to_string = function
    ...
    | Pair (External_alloc, m) ->
      to_string External_alloc ^ "_" ^ maturity_to_string m
    | Pair
  ...
  *)
  let of_string extn_name : t option =
    ...
    | "external_alloc_beta" -> Some (Pair (External_alloc, Beta))
    | _ -> None
end
...
let equal_t (type a b) (a : a t) (b : b t) : (a, b) Misc.eq option =
  ...
  | External_alloc, External_alloc -> Some Refl
